// Declare our custom varying coming from the Vertex Shader
...
vec3 colorRamp(float t) {
    // Blender Color Ramp Stops: Black, Dark Grey, White, Cream
    vec3 c1 = vec3(0.0, 0.0, 0.0);       // Black
    vec3 c2 = vec3(0.2, 0.2, 0.2);       // Dark Grey
    vec3 c3 = vec3(1.0, 1.0, 1.0);       // White
    vec3 c4 = vec3(0.95, 0.9, 0.8);      // Cream/Beige
...
void getAlbedo() {
    ...
    // 3. Color Ramp Node
    vec3 finalColor = colorRamp(noiseVal);

    // 4. Principled BSDF (Output to Surface)
    dAlbedo = finalColor;
}
